// wishlu browser extension logic (firefox)
(function (w, d, n) {
    var $ = w["wishlu"] = {
        'w': w,
        'd': d,
    ...
    //$.w.alert("firefox.js loaded via  chrome extension!!!");
   
    $.f.init("ff");

    self.port.on("rightClick", function (img) {
        $.f.rightClick(img);
    });

    self.port.on("warn", function (msg) {
        $.f.warn();
    });
